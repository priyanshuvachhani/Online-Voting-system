...
<head>
	<title> Edit Home Verification </title>
...
<body>
	...
	<form method="post" enctype="multipart/form-data">
		<div style="padding:1px 16px; position:relative; z-index:1;">
			<h2 align="center">UPDATE HOME VERIFICATION</h2>
			<?php
			if (isset($_GET["no"])) {
			...
			if (isset($_POST["submit"]) && isset($_POST["area"])) {
				...
				if ($row[2] == 0) {
					...
					$qury = mysqli_query($system, "UPDATE home_verification set name='$name' where id=" . $_GET["no"]);
					$qury = mysqli_query($system, "UPDATE home_verification set login_id='$id' where id=" . $_GET["no"]);
					$qury = mysqli_query($system, "UPDATE home_verification set password='$password' where id=" . $_GET["no"]);
					$qury = mysqli_query($system, "UPDATE home_verification set area='$area' where id=" . $_GET["no"]);
					$qury = mysqli_query($system, "UPDATE home_verification set photo='$photo' where id=" . $_GET["no"]);
					if ($qury) {
					...
					echo "<script>
							alert('Voting Started You Cannot Update Home Verification');
						</script>";
				}
			}
			?>
